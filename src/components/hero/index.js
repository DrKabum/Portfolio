...
Hero.ColumnContainer = function HeroColumnContainer({
  children,
  small = false,
  ...props
}) {
  return (
    <div className='hero-column-outter-container'>
      <div
        className={`${
          small ? 'hero-small-column-container' : 'hero-column-container'
        }`}
        {...props}
      >
        {children}
...
Hero.Column = function HeroColumn({ children, spanAllRow = false, ...props }) {
  return (
    <div className={`hero-column ${spanAllRow ? 'all-row' : ''}`} {...props}>
      {children}
    </div>
  )
}
...
Hero.Text = function HeroText({ children, ...props }) {
  return (
    <ReactMarkdown className='hero-text' {...props}>
      {children}
    </ReactMarkdown>
  )
}
...
Hero.Link = function HeroLink({ children, hoverClass = '', href, ...props }) {
  return (
    <a href={href} className={`hero-link ${hoverClass}`} {...props}>
      {children}
    </a>
  )
}
